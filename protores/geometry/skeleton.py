...
class Skeleton:
    ...
    def forward(self, joints_local_rotations, true_hip_offset=None):
        ...
        local_transforms = torch.matmul(self.translation_matrices, rotation_matrices).view(batch_size, joints_nbr, 4, 4) ## 뼈길이와 local rotation matrix를 합친다. 
        ...
        world_transforms[:, 0] = local_transforms[:, 0, :, :] ## world transform과 local transform은 hip에서는 같다. 
